...
def rho_vec(x,gamma):
    absx = np.abs(x)
    return np.where(absx<gamma,-absx**3/3/gamma**2+absx**2/gamma,absx-gamma/3)

def phi_vec(x,gamma):
    absx = np.abs(x)
    return np.where(absx<=gamma,-absx/gamma**2+2/gamma,1/absx)

def psi_vec(x,gamma):
    absx = np.abs(x)
    return np.where(absx<gamma,-1/absx/gamma**2,-1/absx**3)
